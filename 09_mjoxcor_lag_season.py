...
if __name__ == "__main__":
    ...
    for ss, sd in enumerate(seasons_do[:-2]):
        print(sd)
        x = pd.Series(PIO.sel(time=slice(f'{sd:04}-11-01', f'{(sd+1):04}-05-31'))['PRECT_f'][:])
        for vv in range(size_lons):
            y = pd.Series(DS_u['U_timeLon_f'].sel(time=slice(f'{sd:04}-11-01', f'{(sd+1):04}-05-31'))[:,vv])
            corrs = utilities.lagged_correlation(y, x, maxlag)
            xcorr_val_U_timeLon_f[ss, vv, :] = np.array(list(corrs.values()))
        
            y = pd.Series(DS_precip['PRECT_timeLon_f'].sel(time=slice(f'{sd:04}-11-01',f'{(sd+1):04}-05-31'))[:,vv])
            corrs = utilities.lagged_correlation(y, x, maxlag)
    ...
    for ss, sd in enumerate(seasons_do[:-2]):
        ...
        for vv in range(size_lats):
            y = pd.Series(DS_u_lonavg['U_timeLat_f'].sel(time=slice(f'{sd:04}-11-01',f'{(sd+1):04}-05-31'))[:,vv])
            corrs = utilities.lagged_correlation(y, x, maxlag)
            xcorr_val_U_timeLat_f[ss,vv,:] = np.array(list(corrs.values()))
        
            y = pd.Series(DS_precip_lonavg['PRECT_timeLat_f'].sel(time=slice(f'{sd:04}-11-01',f'{(sd+1):04}-05-31'))[:,vv])
            corrs = utilities.lagged_correlation(y, x, maxlag)
